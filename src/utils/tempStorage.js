...
async function createTempDirectory() {
  ...
  for (let i = 0; i < possiblePaths.length; i++) {
    ...
    try {
      ...
      console.log(
        `[TEMP] ✓ Directorio creado exitosamente: ${path.basename(tempDir)}`,
      );
      return tempDir;
    } catch (error) {
      ...
      if (i === possiblePaths.length - 1) {
        throw new Error(
          `No se pudo crear directorio temporal después de ${possiblePaths.length} intentos: ${error.message}`,
        );
      }
    }
  }
...
async function createDirectoryRobust(dirPath) {
  try {
    ...
    try {
      ...
      console.warn(
        `[TEMP] No se pudieron configurar permisos para ${dirPath}: ${chmodError.message}`,
      );
    }
    ...
    console.log(
      `[TEMP] Directorio verificado y funcional: ${path.basename(dirPath)}`,
    );
  } catch (error) {
...
async function cleanupTempFiles() {
  ...
  console.log(
    `[TEMP] Iniciando limpieza de ${tempDirectories.length} directorio(s) temporal(es)...`,
  );
  ...
  const cleanupPromises = tempDirectories.map(async (dir, index) => {
    try {
      ...
      console.log(
        `[TEMP] Limpiando ${index + 1}/${tempDirectories.length}: ${dirName}`,
      );
      ...
      try {
        ...
        console.warn(
          `[TEMP] No se pudo analizar contenido de ${dirName}: ${dirError.message}`,
        );
      }
      ...
      if (stillExists) {
        throw new Error(
          `El directorio ${dirName} aún existe después de la eliminación`,
        );
      }
      console.log(
        `[TEMP] ✓ ${dirName} eliminado (${fileCount} archivos, ${formatBytes(totalSize)})`,
      );
    } catch (error) {
      console.error(
        `[TEMP] ✗ Error limpiando ${path.basename(dir)}: ${error.message}`,
      );
    }
  });
