...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!responseToReturn) {
      console.error("[MAIN] ⚠️ No se generó respuesta, creando respuesta de emergencia");
      const emergencyResult = createEmergencyResult(requestBody, new Error("No response generated"));
...
function createDefaultResult(requestBody) {
  return {
    ...
    NivelDeFormacionSolicitadoParaGrado: requestBody?.Nivel_de_formacion_del_cual_esta_solicitando_grado || 'No especificado',
    ProgramaDelCualSolicita: requestBody?.Programa_del_cual_esta_solicitando_grado || 'No especificado',
    CorreoInsitucional: requestBody?.Correo_electronico_institucional || 'No especificado',
    CorreoPersonal: requestBody?.Correo_electronico_personal || 'No especificado',
    ...
    EK: 'N/A',
    Autorizacion_tratamiento_de_datos: requestBody?.Autorizacion_tratamiento_de_datos || 'No especificado',
...
function createEmergencyResult(requestBody, error) {
  ...
  markAllDocumentsAsError(baseResult, "Error crítico del sistema - Revision Manual");
...
function formatResponse(statusCode, body) {
  ...
  let safeBody;
  try {
    safeBody = typeof body === 'object' && body !== null ? body : { error: "Invalid response body", originalBody: body };
  } catch (bodyError) {
    console.error("[MAIN] Error procesando body de respuesta:", bodyError.message);
    safeBody = { error: "Response body processing failed", details: bodyError.message };
  }
  ...
  if (statusCode >= 400) {
    console.error(`[MAIN] Respuesta de error: ${JSON.stringify(safeBody, null, 2)}`);
  }
